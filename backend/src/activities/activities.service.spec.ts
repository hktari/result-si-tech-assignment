...
describe('ActivitiesService', () => {
  ...
  describe('create', () => {
    it('should create activity successfully with provided timestamp', async () => {
      ...
      // Act
      const actualResult = await activitiesService.create(inputUserId, inputCreateDto);
  ...
  describe('findAll', () => {
    it('should return activities without search filter', async () => {
      ...
      // Act
      const actualResult = await activitiesService.findAll(inputUserId, undefined, inputLimit, inputOffset);
    ...
    it('should return activities with search filter', async () => {
      ...
      // Act
      const actualResult = await activitiesService.findAll(inputUserId, inputSearch, inputLimit, inputOffset);
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException when activity not found', async () => {
      ...
      // Act & Assert
      await expect(activitiesService.findOne(inputId, inputUserId)).rejects.toThrow(NotFoundException);
      expect(mockPrisma.activity.findUnique).toHaveBeenCalledWith({
        where: { id: inputId },
      });
    });
    ...
    it('should throw ForbiddenException when user does not own activity', async () => {
      ...
      // Act & Assert
      await expect(activitiesService.findOne(inputId, inputUserId)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('update', () => {
    it('should update activity successfully', async () => {
      ...
      jest.spyOn(activitiesService, 'findOne').mockResolvedValue(mockExistingActivity);
      mockPrisma.activity.update.mockResolvedValue(expectedUpdatedActivity);

      // Act
      const actualResult = await activitiesService.update(inputId, inputUserId, inputUpdateDto);
