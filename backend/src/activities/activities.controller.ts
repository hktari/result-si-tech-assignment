...
export class ActivitiesController {
  ...
  @Post()
  create(@Request() req, @Body() createActivityDto: CreateActivityDto): Promise<ActivityResponseDto> {
  ...
  @ApiResponse({ status: 200, description: 'Activities retrieved successfully', type: ActivitiesListResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Today\'s activities retrieved successfully', type: [ActivityResponseDto] })
  @Get('today')
  ...
  @ApiResponse({ status: 200, description: 'Activity suggestions retrieved successfully', type: [ActivitySuggestionResponseDto] })
  @ApiQuery({ name: 'q', required: false, description: 'Query for title suggestions' })
  @Get('suggestions')
  getSuggestions(@Request() req, @Query('q') query?: string): Promise<ActivitySuggestionResponseDto[]> {
    return this.activitiesService.getActivitySuggestions(req.user.id, query);
  }

  @ApiOperation({ summary: 'Get a specific activity' })
  @ApiResponse({ status: 200, description: 'Activity retrieved successfully', type: ActivityResponseDto })
  @ApiResponse({ status: 404, description: 'Activity not found' })
  @Get(':id')
  findOne(@Param('id') id: string, @Request() req): Promise<ActivityResponseDto> {
  ...
  @ApiResponse({ status: 200, description: 'Activity deleted successfully', type: DeleteActivityResponseDto })
  @ApiResponse({ status: 404, description: 'Activity not found' })
  @Delete(':id')
  remove(@Param('id') id: string, @Request() req): Promise<DeleteActivityResponseDto> {
