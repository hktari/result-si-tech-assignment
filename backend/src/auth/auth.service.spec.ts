...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [AuthService, mockUsersServiceProvider, mockJwtServiceProvider],
    }).compile();
  ...
  describe('validateUser', () => {
    it('should return user data when credentials are valid', async () => {
      ...
      // Act
      const actualResult = await authService.validateUser(inputEmail, inputPassword);

      // Assert
      expect(actualResult).toEqual(expectedResult);
      expect(mockUsersService.findByEmail).toHaveBeenCalledWith(inputEmail);
      expect(mockedBcrypt.compare).toHaveBeenCalledWith(inputPassword, 'hashedPassword');
    });
  ...
  describe('login', () => {
    it('should return access token and user data when credentials are valid', async () => {
      ...
      // Assert
      expect(actualResult).toEqual(expectedResult);
      expect(authService.validateUser).toHaveBeenCalledWith(inputLoginDto.email, inputLoginDto.password);
    ...
    it('should throw UnauthorizedException when credentials are invalid', async () => {
      ...
      // Act & Assert
      await expect(authService.login(inputLoginDto)).rejects.toThrow(UnauthorizedException);
      expect(authService.validateUser).toHaveBeenCalledWith(inputLoginDto.email, inputLoginDto.password);
  ...
  describe('register', () => {
    it('should create user and return user data without password', async () => {
      ...
      // Assert
      expect(actualResult).toEqual(expectedResult);
      expect(mockedBcrypt.hash).toHaveBeenCalledWith(inputRegisterDto.password, 10);
