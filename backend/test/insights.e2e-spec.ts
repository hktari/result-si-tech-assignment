...
describe('InsightsController (e2e)', () => {
  ...
  describe('/insights?metric=timePerTitle (GET)', () => {
    it('should return time per title insights', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get(
          '/insights?metric=timePerTitle&start=2025-07-15T00:00:00Z&end=2025-07-16T23:59:59Z'
        )
      ...
      const readingInsight = response.body.find(
        item => item.title === 'Reading'
      )
      const writingInsight = response.body.find(
        item => item.title === 'Writing'
      )
      const runningInsight = response.body.find(
        item => item.title === 'Running'
      )
  ...
  describe('/insights?metric=timePerTitleStacked (GET)', () => {
    it('should return stacked time insights for daily interval', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get(
          '/insights?metric=timePerTitleStacked&start=2025-07-15T00:00:00Z&end=2025-07-16T23:59:59Z&interval=daily'
        )
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(200)
    ...
    it('should return stacked time insights for weekly interval', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get(
          '/insights?metric=timePerTitleStacked&start=2025-07-14T00:00:00Z&end=2025-07-20T23:59:59Z&interval=weekly'
        )
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(200)
    ...
    it('should return stacked time insights for monthly interval', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get(
          '/insights?metric=timePerTitleStacked&start=2025-07-01T00:00:00Z&end=2025-07-31T23:59:59Z&interval=monthly'
        )
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(200)
    ...
    it('should default to daily interval when not specified', async () => {
      ...
      const response = await request(app.getHttpServer())
        .get(
          '/insights?metric=timePerTitleStacked&start=2025-07-15T00:00:00Z&end=2025-07-16T23:59:59Z'
        )
        .set('Authorization', `Bearer ${accessToken}`)
        .expect(200)
